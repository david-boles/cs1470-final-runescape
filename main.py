...
def fake_get_data(window_size=10, interp_limit=1, train_set_ratio=0.8):
    """
    interp_limit defines the maximum region that data is allowed
    to be interpolated within. Can be 0 for no interpolation or null
    for no limit. Items for which this condition cannot be met are
...
fake_train_input, fake_train_output, fake_test_input, fake_test_output = fake_get_data()

fake_window_size = fake_train_input.shape[1]
fake_num_items = fake_train_input.shape[2]
fake_num_input_features = fake_train_input.shape[3]
fake_num_output_features = fake_train_output.shape[2]
...
def Fake_ESNModel(loss):
    ...
    model.add(tf.keras.layers.Dense(fake_num_items * 10, activation="relu"))
    model.add(tf.keras.layers.Dense(fake_num_items * 10, activation="relu"))
    model.add(tf.keras.layers.Dense(fake_num_items * 10, activation="relu"))
    model.add(tf.keras.layers.Dense(fake_num_items * 10, activation="relu"))
    model.add(tf.keras.layers.Dense(fake_num_items * fake_num_output_features))
    model.add(tf.keras.layers.Reshape((fake_num_items, fake_num_output_features)))
    opt = tf.keras.optimizers.Adam(learning_rate=1e-4)
    model.compile(
        optimizer=opt,
        loss=loss,
